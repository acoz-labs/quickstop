...
Local package paths shown by `pi list` or stored in settings are relative to the directory containing that scope's settings file. Resolve them against that directory and verify the package before passing absolute `OLD_SOURCE` and `NEW_SOURCE` paths to native remove/install commands at the intended scope. Do not paste a stored relative path into a CLI command that resolves it from the current working directory.
